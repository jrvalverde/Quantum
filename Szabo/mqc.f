...
C*********************************************************************
	...
	IF (IOP .EQ. 0) GO TO 20
	    PRINT 10, N, ZA, ZB
10  	    FORMAT(1H , 2X, 4HSTO-,I1, 21HG FOR ATOMIC NUMBERS , F5.2,5H AND ,
     $      F5.2,//)
20  	CONTINUE
c
...
C	A NORMALIZED 1S SLATER ORBITAL WITH EXPONENT 1.0 IN TERMS OF
...
C
	DO 30 I=1, N
	  DO 30 J = 1, N
		...
		RPQ2 = RPQ * RPQ
		V1111 = V1111 + 
     $			TWOE(A1(I), A1(J), A1(K), A1(L), 0.0D0, 0.0D0, 0.0D0) 
     $			* D1(I) * D1(J) * D1(K) * D1(L)
     		V2111 = V2111 +
     $			TWOE(A2(I), A1(J), A1(K), A1(L), R2, 0.0D0, RAP2)
     $			* D2(I) * D1(J) * D1(K) * D1(L)
     		V2121 = V2121 +
     $			TWOE(A2(I), A1(J), A2(K), A1(L), R2, R2, RPQ2)
     $			* D2(I) * D1(J) * D2(K) * D1(L)
     		V2211 = V2211 +
     $			TWOE(A2(I), A2(J), A1(K), A1(L), 0.0D0, 0.0D0, R2)
     $			* D2(I) * D2(J) * D1(K) * D1(L)
     		V2221 = V2221 +
     $	 		TWOE(A2(I), A2(J), A2(K), A1(L), 0.0D0, R2, RBQ2)
     $			* D2(I) * D2(J) * D2(K) * D1(L)
     		V2222 = V2222 +
     $			TWOE(A2(I), A2(J), A2(K), A2(L), 0.0D0, 0.0D0, 0.0D0)
     $			* D2(I) * D2(J) * D2(K) * D2(L)
30	CONTINUE
...
C*********************************************************************
	...
	DIMENSION A(5)
	DATA P/0.3275911D0/
	DATA A/ 0.25829592D0, -0.284496736D0, 1.421413741D0,
     $		-1.453152027D0, 1.061405439D0/
	T = 1.0D0 / (1.0D0 + P * ARG)
	TN = T
	POLY = A(1) * TN
	DO 10 I = 2, 5
	  ...
	  POLY = POLY + A(I) * TN
10	CONTINUE
	DBLERF = 1.0D0 - POLY * DEXP(-ARG * ARG)
	RETURN
	END
...
C*********************************************************************
	IMPLICIT DOUBLE PRECISION(A-H,O-Z)
	DATA PI/3.1415926535898D0/
	T = A * B / (A + B) * (3.0D0 - 2.0D0 * A * B * RAB2 / (A + B))
     $	    * (PI / (A + B)) ** 1.5D0 * DEXP(-A * B * RAB2 /(A + B))
	RETURN
	END
...
C	RELEVANT MATRICES, THAT IS, S, H, X, XT, AND TWO-ELECTRON 
C	INTEGRALS
C
C*********************************************************************
	IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C
C	START ITERATION LOOP (JUMP BACK HERE IF NOT CONVERGED/EXHAUSTED)
C
20	ITER = ITER + 1
...
120	CONTINUE
	...
	IF (IOP .EQ. 0) GO TO 140
	    PRINT 130, DELTA
130	    FORMAT(/, 4X, 39HDELTA(CONVERGENCE OF DENSITY MATRIX) = , F10.6,/)
140	CONTINUE
C
