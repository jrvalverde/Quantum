...
for j in 8ogtp oxg gn gtp ; do
    cd $j
    for i in [123]* ; do
	if [ -s $i/rigid.out ] ; then continue ; fi
	cd $i
	echo $j/$i
	rm -f core rigid* STD*
        cat > rigid.in <<END
...
atom_model                                                   all
vdw_defn_file                                                $HOME/contrib/dock6/parameters/vdw_AMBER_parm99.defn
flex_defn_file                                               $HOME/contrib/dock6/parameters/flex.defn
flex_drive_file                                              $HOME/contrib/dock6/parameters/flex_drive.tbl
...
#   this is for our simpler queue system
	qsub -q slow -e `pwd` -o `pwd` -N RIGID <<ENDSUB
#!/bin/bash
#    	    cd $HOME/work/arrojas/dock6/$j/$i
#	    echo `pwd` $HOME
#	    export PATH=$HOME/dock6/bin
#	    dock6 -i rigid.in -o rigid.out
	    cd $HOME/work/arrojas/newdock6/$j/$i
	    echo `pwd` $HOME
	    $HOME/contrib/dock6/bin/dock6 -i rigid.in -o rigid.out
ENDSUB
    cd ..
    done
    cd ..
